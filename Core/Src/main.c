...
extern uint8_t metric_change_bool = 0;
extern uint8_t curr_metric = 0;
uint8_t distance;
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	//*lidar_tx_buffer = {0x5A, 0x06, 0x03, 0x00, 0x00, 0x00};
	//*lidar_tx_buffer = 0x5A040400;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(LEFT_MOTOR_SPEED_GPIO_Port, LEFT_MOTOR_SPEED_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(RIGHT_MOTOR_IA4_GPIO_Port, RIGHT_MOTOR_IA4_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, RIGHT_MOTOR_SPEED_Pin|GPIO_PIN_13|LCD_TE_Pin|WIFI_WKUP_Pin
  ...
  /*Configure GPIO pins : RIGHT_MOTOR_SPEED_Pin PB13 LCD_TE_Pin WIFI_WKUP_Pin
                           LEFT_MOTOR_I_Pin */
  GPIO_InitStruct.Pin = RIGHT_MOTOR_SPEED_Pin|GPIO_PIN_13|LCD_TE_Pin|WIFI_WKUP_Pin
                          |LEFT_MOTOR_I_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void StartChangeMetricTask(void *argument)
{
  /* USER CODE BEGIN StartChangeMetricTask */
	...
	// cm -> inch -> feet -> meter
	for (;;) {
		if (metric_change_bool == 1)
		{
			//Changing from cm to inch
			if (curr_metric == 0)
			{
				distance *= 0.393701;
			}
			...
			//Changing from feet to meter
			else if (curr_metric == 2)
			{
				distance *= 0.3048;
			}
			//Changing from meter to cm
			else if (curr_metric == 3)
			{
				distance *= 100;
			}
			metric_change_bool = 0;
		}
	}
