...
const osThreadAttr_t MetricChange_attributes = {
  .name = "MetricChange",
...
const osThreadAttr_t Lidar_attributes = {
  .name = "Lidar",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};
...
#define LIDAR_TASK_RATE 200
#define LCD_TASK_RATE 500
...
int main(void)
{
  ...
  MetricChangeHandle = osThreadNew(MetricChangeTask, NULL, &MetricChange_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOE, LED1_RED_Pin|MEMS_LED_Pin|LCD_BL_CTRL_Pin|LEFT_MOTOR_2_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, RIGHT_MOTOR_I_Pin|RIGHT_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(RIGHT_MOTOR_IA4_GPIO_Port, RIGHT_MOTOR_IA4_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13|LCD_TE_Pin|WIFI_WKUP_Pin|LEFT_MOTOR_I_Pin, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin = LED1_RED_Pin|MEMS_LED_Pin|LCD_BL_CTRL_Pin|LEFT_MOTOR_2_Pin;
  ...
  GPIO_InitStruct.Pin = GPIO_PIN_13|LCD_TE_Pin|WIFI_WKUP_Pin|LEFT_MOTOR_I_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  ...
  /* USER CODE BEGIN MotorTask */
	...
	for (;;) {
		...
		if (distance_high > 0) {
			...
			__HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_4, 600);

			//MOTOR A
			//HAL_GPIO_WritePin(GPIOF, GPIO_PIN_10, GPIO_PIN_SET); //PWM
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_SET);

			//MOTOR B
			//HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET); //PWM
			HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);
		} else if (distance_low > 40) {
			//delete 1st
			HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, GPIO_PIN_SET);

			__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_3, distance_low * 2);
			__HAL_TIM_SET_COMPARE(&htim5, TIM_CHANNEL_4, distance_low * 2);
...
void LidarTask(void *argument)
{
  /* USER CODE BEGIN LidarTask */
	...
	for (;;) {
		...
		HAL_StatusTypeDef statusR = HAL_UART_Receive(&huart7, lidar_rx_buffer,
				9, 100);
